...
export default function DirectoryClient({ initialShops, total: initialTotal, categories, initialQuery, initialCategory, initialLocation }: any) {
  const router = useRouter()
  const [shops, setShops] = useState<any[]>(initialShops.map(toCard))
  ...
  useEffect(() => {
    // Bỏ qua lần fetch đầu tiên nếu state trùng với params từ server
    if (isFirstRender.current) {
      isFirstRender.current = false
      if (page === 1 && searchTerm === initialQuery && location === initialLocation && category === initialCategory) {
        return
      }
    }
    ...
    const timeout = window.setTimeout(async () => {
      ...
      try {
        ...
        router.replace(`/directory?${query.toString()}`, { scroll: false })
  ...
  }, [page, searchTerm, location, category, initialQuery, initialLocation, initialCategory, router])

  const categoryItems = useMemo(() => [{ id: ALL, label: ALL, icon: 'Box' }, ...categories.map((item: any) => ({ id: item.name, label: item.name, icon: item.icon }))], [categories])
  const pageSize = 24
  ...
  return <main className="min-h-screen bg-white pb-16 pt-24 sm:pt-32"><div className="mx-auto max-w-7xl space-y-7 px-4 sm:space-y-10 sm:px-6">
    <header className="text-center"><h1 className="text-4xl font-bold text-[#2F2F2F] sm:text-5xl md:text-7xl">Danh bạ <span className="text-[#D4AF37]">Shop.</span></h1><p className="mt-4 text-xs font-bold uppercase tracking-widest text-[#2F2F2F]/50 sm:mt-5">{total.toLocaleString('vi-VN')} shop phù hợp</p></header>
